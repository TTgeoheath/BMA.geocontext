#' model.pp
#'
#' This function calculates the coefficients (i.e.,beta, p-value, z-value, standard error, hazard ratio,
#' confidence interval), posterior possibility, and BIC for each exposure-specific cox model.
#'
...
model.pp <- function(data,surv_time,status,covariates,exposure_colnames) {
  ...
  # surv_time: the column name of survivial time, for instance, "survival_time".
  ...
  if (length(prefixes) < 2)
    stop("The number of exposures must be more than 2 !")
  ...
  ## the calculation is based on the equation The code you provided calculates the posterior probabilities of different
  # model combinations using Bayes' rule, assuming that all model combinations have equal prior probabilities.
...
##extract the higher ci and lower ci for exposures.
  ...
  lower_ci <- t(lower_ci)
  low_ci_names <- paste0(prefixes, c("_lower_ci"))
  ...
  high_ci_names <- paste0(prefixes, c("_higher_ci"))
  ci_names <- c(low_ci_names,high_ci_names)
  ci <- cbind(lower_ci,higher_ci)
  colnames(ci) <- ci_names
...
models <- t(data.frame(Model = model_combinations))
bics <- data.frame(Model_bics = model_bics)
  # Step 6: Create a data frame with the model combinations, posterior probabilities, hazard ratios, and confidence intervals
  result <- cbind(models,coef,ci)
  result <- cbind(result,posterior_probs)
  result <- cbind(result,bics)
  number.models <- 1:length(model_combinations)
  number.models <- paste0(c("model."),number.models)
  row.names(result) <- number.models
  return(result)
}
